...
local can joebiden petebuttigieg berniesanders elizabethwarren amyklobuchar mikebloomberg
...
import excel "C:\Users\39174\Desktop\neoliberal\data\JulySeptOctNovDecJanFebFeb.xlsx", sheet("Weighted") firstrow
...
label variable MikeBloomberg "Bloomberg"

graph twoway line ( JoeBiden PeteButtigieg AmyKlobuchar BernieSanders ElizabethWarren) Month, ytitle("Average Ranking") ysc(reverse) title("All Candidates") scheme(exponentsmag) r2title("← Less favored — More favored →") xlabel(, format(%tdMon))
...
graph twoway line (BernieSanders ElizabethWarren) Month, ytitle("Average Ranking") ysc(reverse) title("Progressives and Outsiders") scheme(exponentsmag) r2title("← Less favorable — More favorable →") xlabel(, format(%tdMon))
